...
namespace Hooks
{
	...
	long __stdcall hkEndScene(IDirect3DDevice9* pDevice)
	{
		...
		static auto fov = g_CVar->FindVar("fov_cs_debug");
		static auto no_visual_recoil = g_CVar->FindVar("view_recoil_tracking");
		...
		if (g_Options.misc_no_visual_recoil)
			no_visual_recoil->SetValue(0);
		else
			no_visual_recoil->SetValue(0.45f);

		viewmodel_fov->m_fnChangeCallbacks.m_Size = 0;
		viewmodel_fov->SetValue(g_Options.viewmodel_fov);
		fov->SetValue(g_Options.fov);
	...
	void __stdcall hkCreateMove(int sequence_number, float input_sample_frametime, bool active, bool& bSendPacket)
	{
		...
		if (g_Options.Aimbot_BackTrack) {
			Backtrack.BackTrackStart(cmd);
			Backtrack.BackTrackingCalc(cmd);
		}
