...
void Skins::OnFrameStageNotify(ClientFrameStage_t stage)
{
		...
		return;
	{
		...
		const auto glove_config = &skins.m_items[GLOVE_MOTORCYCLE];
	...
	}
	{
		...
		for (int i = 0; weapons[i].IsValid(); i++)
		{
			...
			const auto active_conf = &skins.m_items[is_knife(definition_index) ? WEAPON_KNIFE : definition_index];
